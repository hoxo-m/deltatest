...
In such settings, **naive tests can underestimate uncertainty**—for example, **standard Z-tests**, **chi-squared tests**, or **tests for differences in proportions**—because observations within a user are not independent. `deltatest` applies a delta-method-based variance estimator to account for this issue.
...
ggplot(df, aes(p, prop)) +
  ...
  scale_y_continuous(breaks = seq(0, 1, by = 0.05), limits = c(0, 0.1)) +
...
p_values <- readRDS("data-raw/p_values_from_Delta_method.rds")
...
By default, `deltatest` tests the difference between two groups. If you specify `type = "relative_change"`, it tests the rate of change, i.e., (μ_t - μ_c) / μ_c where μ_c and μ_t represent the mean values of the control group and the treatment group, respectively.
