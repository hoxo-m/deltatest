...
#' @param lambda numeric value specifying the Poisson rate parameter for the
#'   number of sessions and page-views generated by a single user.
...
generate_dummy_data <- function(
    n_user, model = c("Bernoulli", "normal"), xi = 0, sigma = 0, lambda = 3,
  ...
  # generate data -----------------------------------------------------------
  n_session <- rpois(n_user, lambda) + 1L
  n_pageview <- n_session |> lapply(function(x) rpois(x, lambda) + 1L) |> unlist()
  ...
  result <- data.frame(user_id, group, metric)
